...
export default function StonePortfolio() {
  return (
    <>
      ...
      <section className="bg-[#f7f3ec]">
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2" data-reveal>
            ...
            <span className="text-sm text-[#1f1b16]/45">{MATERIALS.join(" · ")}</span>
          </div>
          <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" data-stagger>
            {PORTFOLIO.map((w) => (
              <figure key={w.src} className="group overflow-hidden rounded-2xl border border-[#1f1b16]/10 bg-white">
                ...
                <figcaption className="p-5">
                  <h3 className={`${fd} text-lg font-semibold text-[#1f1b16]`}>{w.t}</h3>
                  <p className="mt-1.5 text-sm font-light leading-relaxed text-[#1f1b16]/60">{w.b}</p>
                </figcaption>
              </figure>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="border-t border-[#1f1b16]/10 bg-[#efe7da]">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-5 py-14 text-center md:flex-row md:px-8 md:text-left" data-reveal>
          <h2 className={`max-w-xl ${fd} text-2xl font-medium leading-tight tracking-tight text-[#1f1b16] md:text-4xl`}>Got a piece that needs the same care?</h2>
