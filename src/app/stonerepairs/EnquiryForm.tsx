...
const field =
  "w-full rounded-lg border border-[#1f1b16]/15 bg-white px-4 py-3 text-[#1f1b16] placeholder-[#1f1b16]/40 outline-none transition-shadow focus:border-[#9a7a48] focus:ring-2 focus:ring-[#9a7a48]/20";
...
export function EnquiryForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3.5">
      ...
      <select name="item" defaultValue="" className={`${field} text-[#1f1b16]/70`} required>
        <option value="" disabled>What needs repairing?</option>
        {ITEMS.map((s) => (<option key={s} value={s} className="text-[#1f1b16]">{s}</option>))}
      </select>
      ...
      <button type="submit" className="group flex w-full items-center justify-center gap-2 rounded-full bg-[#1f1b16] px-6 py-4 text-sm font-semibold text-[#f7f3ec] transition-colors hover:bg-[#9a7a48]">
        Send enquiry
        <span className="transition-transform duration-300 group-hover:translate-x-1">→</span>
      </button>
      <p className="text-center text-xs text-[#1f1b16]/45">
