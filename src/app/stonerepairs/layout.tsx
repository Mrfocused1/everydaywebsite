...
export default function SRLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className={`${display.variable} ${body.variable} bg-[#f7f3ec] text-[#1f1b16]`} style={{ fontFamily: "var(--font-sr-body), sans-serif" }}>
      <SRHeader />
      <main>{children}</main>
      <SRFooter />
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 flex gap-2 border-t border-white/10 bg-[#1f1b16] p-3 shadow-[0_-4px_20px_rgba(0,0,0,0.25)] sm:inset-x-auto sm:bottom-5 sm:right-5 sm:rounded-full sm:border sm:border-white/10 sm:p-2 sm:shadow-[0_12px_40px_rgba(31,27,22,0.35)]">
        <a href={PHONE_TEL} className="flex-1 rounded-full border border-white/30 px-5 py-3 text-center text-sm font-semibold text-[#f7f3ec] transition-colors hover:bg-white hover:text-[#1f1b16] sm:flex-none sm:px-6">
          Call now
        </a>
        <Link href="/stonerepairs/contact#enquire" className="flex-1 rounded-full bg-[#9a7a48] px-5 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-[#b08a52] sm:flex-none sm:px-6">
          Enquire
        </Link>
      </div>
    </div>
