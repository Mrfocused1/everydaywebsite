...
import {
  IMG, PRODUCTS, MARQUEE, TRUST, CLIENTS, AccentBar, fd, Label, Arrow,
...
export default function IBHome() {
  return (
    <>
      {/* ── Hero ── */}
      <section className="relative isolate flex min-h-[90vh] items-end overflow-hidden bg-[#0e0e10]">
        <img src={IMG.hero} alt="Custom branded garments" data-parallax className="absolute inset-0 -z-10 h-[112%] w-full object-cover opacity-80" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-t from-black via-black/55 to-black/30" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-r from-black/70 via-transparent to-transparent" />
        <AccentBar onDark className="absolute inset-x-0 top-0 z-10" />
        <div className="mx-auto w-full max-w-6xl px-5 pb-16 pt-32 text-white md:px-8 md:pb-24">
          <div className="max-w-3xl">
            ...
            <h1 className={`hero-h mt-6 ${fd} text-[3.4rem] uppercase leading-[0.86] tracking-tight md:text-[6rem]`}>
              ...
              <span className="block overflow-hidden"><span className="hero-line-inner block text-white/55" style={{ transform: "translateY(110%)" }}>Wear it.</span></span>
            </h1>
            <p className="hero-fade mt-7 max-w-xl text-lg font-light text-white/80 opacity-0" style={{ transform: "translateY(8px)" }}>
            ...
            <div className="hero-fade mt-10 flex flex-wrap gap-x-7 gap-y-2 border-t border-white/20 pt-6 text-xs font-bold uppercase tracking-wider opacity-0">
              {TRUST.map((t) => (
                <span key={t} className="flex items-center gap-2 text-white/75"><span className="h-1.5 w-1.5 bg-white" aria-hidden /> {t}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* ── Marquee ── */}
      <section className="overflow-hidden border-y border-white/10 bg-[#0e0e10] py-5 text-white">
        <div data-marquee-track className="flex w-max items-center gap-8 whitespace-nowrap will-change-transform">
          {[...MARQUEE, ...MARQUEE].map((m, i) => (
            <span key={i} className={`flex items-center gap-8 ${fd} text-2xl uppercase tracking-tight md:text-3xl`}>
              {m} <span className="h-2.5 w-2.5 bg-white" aria-hidden />
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="relative isolate overflow-hidden bg-[#0e0e10] text-white">
        <img src={IMG.statement} alt="" aria-hidden data-parallax className="absolute inset-0 -z-10 h-[112%] w-full object-cover opacity-30" />
        <div className="absolute inset-0 -z-10 bg-black/65" />
        <div className="mx-auto max-w-6xl px-5 py-20 md:px-8 md:py-28" data-reveal>
          <AccentBar onDark className="max-w-[12rem]" />
          <p className={`mt-7 ${fd} text-[2.6rem] uppercase leading-[0.92] tracking-tight md:text-[4.6rem]`}>
            From a single tee<br />to ten thousand.
          </p>
          <p className="mt-5 max-w-xl text-lg font-light text-white/65">No minimum order, no fuss. Order one or order thousands — same quality, same care.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          <div className="flex flex-wrap items-end justify-between gap-4" data-reveal>
            <div className="max-w-2xl">
              <Label>What we brand</Label>
              <h2 className={`mt-5 ${fd} text-4xl uppercase leading-[0.95] tracking-tight md:text-5xl`}>Garments &amp; merch.</h2>
          ...
          <div className="-mx-5 mt-10 flex snap-x snap-mandatory gap-5 overflow-x-auto px-5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:mt-14 sm:grid sm:grid-cols-2 sm:gap-6 sm:overflow-visible sm:px-0 lg:grid-cols-4" data-stagger>
            {PRODUCTS.map((p) => (
              <article key={p.t} className="group relative flex w-[78%] shrink-0 snap-start flex-col overflow-hidden bg-white ring-1 ring-[#0e0e10]/10 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_22px_45px_-22px_rgba(0,0,0,0.45)] sm:w-auto">
                <div className="relative overflow-hidden">
                ...
                <div className="flex flex-1 flex-col p-5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="-translate-x-1 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100" aria-hidden>→</span>
                  </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 sm:mt-14 md:grid-cols-3 md:gap-8" data-stagger>
            {STEPS.map((s) => (
              <div key={s.n} className="group border border-[#0e0e10]/12 p-7 transition-colors hover:bg-[#0e0e10] hover:text-white">
                <p className={`${fd} text-6xl leading-none text-[#0e0e10] group-hover:text-white`}>{s.n}</p>
                <h3 className={`mt-4 ${fd} text-xl uppercase tracking-tight`}>{s.t}</h3>
                <p className="mt-2 font-light text-slate-600 group-hover:text-white/70">{s.b}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-5 py-16 text-center md:px-8 md:py-20" data-reveal>
          <p className="text-sm font-bold uppercase tracking-[0.25em] text-slate-400">Brands we&apos;ve printed for</p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
            {CLIENTS.map((c) => (
              <span key={c} className={`${fd} text-2xl uppercase tracking-tight text-[#0e0e10] md:text-4xl`}>{c}</span>
            ))}
          </div>
          <Link href="/industry-branding/work" className="mt-9 inline-block text-sm font-bold uppercase tracking-wide text-[#0e0e10] underline-offset-4 hover:underline">View our work →</Link>
        </div>
      </section>
            ...
            <ul className="mt-7 grid gap-x-6 gap-y-3 sm:grid-cols-2">
              {REASONS.map((r) => (
                <li key={r} className="flex items-start gap-3 text-sm font-medium text-[#0e0e10]">
                  <span className="mt-1 h-2 w-2 shrink-0 bg-[#0e0e10]" aria-hidden /> {r}
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-5 pb-20 md:px-8" data-reveal>
          <div className="relative flex flex-col items-center justify-between gap-6 overflow-hidden bg-[#0e0e10] px-7 py-12 text-center text-white md:flex-row md:px-12 md:text-left">
            ...
            <div>
              ...
              <p className="mt-2 font-light text-white/75">Tell us what you need branded — no minimum order, fast turnaround.</p>
            </div>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <a href={INSTAGRAM} target="_blank" rel="noopener noreferrer" className="rounded-md border border-white/40 px-7 py-4 text-sm font-bold uppercase tracking-wide text-white transition-colors hover:bg-white/10">DM on Instagram</a>
            </div>
          </div>
        </div>
      </section>
