...
import { NAV, TRUST, SOCIALS, PHONE_DISPLAY, PHONE_TEL, EMAIL, ADDRESS, LOCATIONS, HOURS } from "./brand";

export function IBFooter() {
  return (
      ...
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1.3fr]">
          <div>
            ...
            <img src="/marketing/industry-branding-logo.png" alt="Industry Branding" className="h-14 w-auto" />
            <p className="mt-5 max-w-sm text-sm font-light text-white/45">
              High-quality, cost-effective garment printing &amp; branding with a fast turnaround. Custom clothing and
              merch, no minimum order, based in {LOCATIONS}.
            </p>
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-semibold uppercase tracking-wider text-white/40">
              {TRUST.map((t) => (
                <span key={t} className="flex items-center gap-2"><span className="h-1.5 w-1.5 bg-white" aria-hidden /> {t}</span>
              ))}
            </div>
          </div>
          <div>
        ...
        <div className="mt-12 flex flex-col gap-2 border-t border-white/10 pt-6 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 Industry Branding. All rights reserved.</p>
