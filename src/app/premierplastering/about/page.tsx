...
export default function PPAbout() {
  return (
    <>
      ...
      {/* Stats */}
      <section className="bg-[#16293c] text-white">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-px bg-white/10 md:grid-cols-4" data-stagger>
          {[
            ...
            <div key={st.l} className="bg-[#16293c] p-7 text-center md:p-10">
              <p className="text-4xl font-bold tracking-tight text-[#9ec9ee] md:text-5xl" data-count={st.v} data-suffix={st.s}>0{st.s}</p>
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 sm:mt-14 sm:grid-cols-2 lg:grid-cols-4" data-stagger>
            {VALUES.map((v) => (
              <div key={v.t} className="border-t-2 border-[#ff5e3c] bg-white pt-5">
                <h3 className="text-lg font-bold uppercase tracking-tight text-[#16293c]">{v.t}</h3>
        ...
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          <div className="grid gap-12 md:grid-cols-2 md:gap-16">
            <div data-reveal>
              ...
              <div className="mt-8 grid grid-cols-2 border border-[#16293c]/15 bg-white">
                {ACCREDS.map((a) => (
                  <div key={a} className="flex flex-col gap-3 border-b border-r border-[#16293c]/10 p-6 odd:border-r [&:nth-child(n+3)]:border-b-0">
            ...
            <div data-reveal>
              ...
              <div className="mt-8 grid grid-cols-2 gap-x-8">
                {[AREAS.slice(0, 4), AREAS.slice(4)].map((col, ci) => (
                  <div key={ci} className="flex flex-col divide-y divide-[#16293c]/10 border-y border-[#16293c]/15">
                    {col.map((c) => (
                      <div key={c} className="flex items-center gap-3 py-4">
                        <span className="h-2 w-2 shrink-0 bg-[#ff5e3c]" aria-hidden />
                        <p className="text-base font-bold uppercase tracking-wide text-[#16293c]">{c}</p>
                      </div>
                    ))}
                  </div>
                ))}
        ...
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 sm:mt-14 sm:grid-cols-4" data-stagger>
            {[GALLERY[1], GALLERY[4], GALLERY[2], GALLERY[16]].map((g) => (
              <figure key={g.src} className="relative aspect-[3/4] overflow-hidden border border-[#16293c]/10">
