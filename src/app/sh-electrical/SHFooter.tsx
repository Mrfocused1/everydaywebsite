      ...
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        <div className="grid gap-10 md:grid-cols-[1.5fr_1fr_1.3fr]">
          <div>
            ...
            <img src="/marketing/sh-electrical-logo.png" alt="SH Electrical Mcr Ltd" className="h-11 w-auto" />
            <p className="mt-5 max-w-sm text-sm font-light text-white/50">
              A family-run, NAPIT-approved electrician serving {AREA} with over 30 years&apos; experience. Domestic,
              commercial and industrial — done safely, certified and to a high standard.
            </p>
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-semibold uppercase tracking-wider text-white/40">
              {ACCREDS.map((a) => (
                <span key={a} className="flex items-center gap-2"><span className="h-1.5 w-1.5 bg-[#1f8fd6]" aria-hidden /> {a}</span>
              ))}
            </div>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm uppercase tracking-wide">
              {NAV.map((n) => (
                <li key={n.href}>
                  <Link href={n.href} className="text-white/65 transition-colors hover:text-[#1f8fd6]">{n.label}</Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 flex flex-col gap-2 border-t border-white/10 pt-6 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 SH Electrical Mcr Ltd. All rights reserved.</p>
